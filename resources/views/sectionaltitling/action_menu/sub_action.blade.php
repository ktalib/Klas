<div class="relative dropdown-container">
   ...
   <ul class="fixed action-menu z-50 bg-white border rounded-lg shadow-lg hidden w-56">
      ...
      <li>
         ...
         onclick="openERegistryModal(
            ...
            @if($app->applicant_type == 'individual')
               {{ json_encode(['applicant_title' => $app->applicant_title, 'first_name' => $app->first_name, 'surname' => $app->surname]) }}
            @elseif($app->applicant_type == 'corporate')
               {{ json_encode(['corporate_name' => $app->corporate_name]) }}
            @elseif($app->applicant_type == 'multiple')
      ...
      <li>
         ...
         @if(!in_array($app->planning_recommendation_status, ['Approved']) || 
           !in_array($app->application_status, ['Approved'])) 
